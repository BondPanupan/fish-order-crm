...
export default function SupplierManager() {
  ...
  function openEdit(s: Supplier) {
    setEditTarget(s);
    setForm({ code: s.code, name: s.name ?? '', isWildcard: s.isWildcard });
    setFormError(null);
    setModal('edit');
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (modal === 'edit') await updateSupplier(editTarget!.id, dto);
      else await createSupplier(dto);
      closeModal(); loadAll();
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <button onClick={openCreate} className={styles.btnPrimary}>+ New Supplier</button>
      </div>
      ...
      {!loading && !error && suppliers.length === 0 && <p className={styles.muted}>No suppliers yet.</p>}
            ...
            <tbody>
              {suppliers.map((s, i) => (
                <tr key={s.id} className={i % 2 === 0 ? styles.trEven : styles.trOdd}>
                  <td className={styles.td}><span className={styles.code}>{s.code}</span></td>
                  <td className={styles.td}>{s.name ?? '—'}</td>
                  <td className={`${styles.td} ${styles.tdCenter}`}>{s.isWildcard ? 'Yes' : 'No'}</td>
        ...
        <div className={styles.overlay} onClick={closeModal}>
          <div className={styles.dialog} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <label className={styles.label}>
                Name
                <input className={styles.input} value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Supplier name" />
              </label>
